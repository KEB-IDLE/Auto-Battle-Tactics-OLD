...
public class GamePlayController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        PrewarmProjectilesFromGameData();
    ...
    // GamePlayController.cs 내부 유틸
    public GameObject PlayEffect(GameObject prefab, Transform attach, float duration)
    {
        var go = SpawnFromPool(prefab, attach.position, Quaternion.identity, attach);
        StartCoroutine(ReturnAfter(go, duration));
        return go;
    }
    ...
    private void PrewarmProjectilesFromGameData()
    {
        if (gameData == null || gameData.championsArray == null) return;
        foreach (var c in gameData.championsArray)
        {
            if (c != null && c.attackProjectile != null)
                Prewarm(c.attackProjectile, defaultPrewarm);
        }
    }
    ...
    [Header("Pooling")]
    [SerializeField] private Transform poolRoot;   // 없으면 런타임에 생성
    ...
    // GamePlayController.cs 내부 (메서드 영역)
    ...
    public GameObject SpawnFromPool(GameObject prefab, Vector3 pos, Quaternion rot, Transform parent = null)
    {
        ...
        //if (parent != null) go.transform.SetParent(parent, worldPositionLost: true);
    ...
    public void StartDrag()
    {
        ...
        TriggerInfo triggerinfo = inputController.triggerInfo ?? RaycastTriggerFromMouse();
    ...
    /// </summary>
    /// <returns></returns>
    // GamePlayController.cs 내부 아무 곳(예: private 메서드 영역)에 추가
    private TriggerInfo RaycastTriggerFromMouse()
    {
        var cam = Camera.main;
        ...
        {
            var ti = h.collider.GetComponent<TriggerInfo>();
